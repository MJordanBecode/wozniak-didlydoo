...
export default async function modalLog(eventId) {
    ...
    filterButton.classList.add("btn", "btn-filter");
    ...
    exportButton.classList.add("btn", "btn-export");
    ...
    refreshButton.classList.add("btn", "btn-refresh");
    ...
    if (logs && logs.length > 0) {
        logs.forEach((log, index) => {
            ...
            tr.innerHTML = `
                <td><a href="#" class="log-id">#${log.id || index + 1000}</a></td>
                <td>${formattedDate} <span class="time-badge">${formattedTime}</span></td>
                <td>${formattedDate} <span class="time-badge">${formattedTime}</span></td>
                <td><div class="code-block">${changedElements}</div></td>
        ...
        const tr = document.createElement("tr");
        tr.innerHTML = '<td colspan="5" class="no-data">Aucun log trouvé.</td>';
        tbody.appendChild(tr);
    }
    ...
    const pageButtons = paginationControls.querySelectorAll(".page-btn:not(.prev-btn):not(.next-btn)");
